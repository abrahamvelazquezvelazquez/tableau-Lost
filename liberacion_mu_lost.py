...
def liberacion_mu_lost():
    ...
    # 1. Obtener encabezados de la Fila 1 local
    ...
    if col_key_idx == 0 or col_inbound_idx == 0:
        print(
            "Error: No se encontró la columna 'Key' o 'Inbound' en la fila 1 de la hoja principal."
        )
        return

    # Leer los valores de la columna "Key" desde la fila 2 en adelante
    keys_locales = sheet.col_values(col_key_idx)[1:]
    if not keys_locales:
        print("No hay datos en la columna Key para procesar.")
        return

    # 2. Leer datos de la pestaña TC
    ...
    # 3. Crear mapa de búsqueda: Clave = Columna A (índice 0) -> Valor = Columna C (índice 2)
    lookup = {}
    for row in data_ext:
        if len(row) > 0:
    ...
    for item in keys_locales:
        ...
        valor_encontrado = lookup.get(key_val, "")
        output_inbound.append([valor_encontrado])

    # 5. Escribir resultados en la columna "Inbound"
    ...
    print(
        f"Se actualizaron {num_rows} filas en la columna Inbound de Liberación MU Lost exitosamente."
    )
